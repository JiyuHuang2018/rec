...
if __name__ == '__main__':
    ...
    model_name = 'wg_pmf_cau_user_add'
    ...
    for dim in dims:
        ...
        for i, K in enumerate(outdims):
            ...
            gamma = Normal(loc=tf.zeros([M, 1]), scale=pri_V*tf.ones([M, 1]))
            ...
            qgamma = PointMass(params=tf.gather(qgamma_variables[0], idx_ph))
            ...
            pred = sparse.csr_matrix(U_out.dot(V_out.T) + \
                                     np.multiply(np.repeat(gamma_out, reconstr_cau.shape[1], axis=1), \
                                                 reconstr_cau) + beta0_out)
