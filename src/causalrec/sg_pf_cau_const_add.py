...
# new
    ...
    model_name = 'sg_pf_cau_const_add'
    ...
    for dim in dims:
        ...
        for i, K in enumerate(outdims):
            ...
            qgamma = PointMass(params=tf.nn.softplus(qgamma_variables[0]))
            ...
            inference_gamma.initialize(optimizer=optimizer, scale={x: scale_factor, U: scale_factor},
                                 var_list=qgamma_variables)
            inference_beta0.initialize(optimizer=optimizer, scale={x: scale_factor, U: scale_factor},
            ...
            gamma_out = tf.nn.softplus(qgamma_variables[0]).eval()
            ...
            # np.savetxt(OUT_DATA_DIR + '/'+ out_filename + "_cas_gamma.csv", gamma_out)
            ...
            inference_U.initialize(optimizer=optimizer, scale={x: scale_factor, U: scale_factor},
                                 var_list=qU_variables, n_iter=n_iter)
