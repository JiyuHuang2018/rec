...
if __name__ == '__main__':
    ...
    for dim in dims:
        ...
        for i, K in enumerate(outdims):
            ...
            U_trainout, V_out = pf.Eb.copy(), pf.Et.T
            np.savetxt(OUT_DATA_DIR + '/'+ model_name + '_bin'+str(binary)+'_cauk0_'+str(caudim)\
                ...
                "_alpha"+str(args.alpha)+"_cas_trainU.csv", U_trainout)
            np.savetxt(OUT_DATA_DIR + '/'+ model_name + '_bin'+str(binary)+'_cauk0_'+str(caudim)\
                +'outK'+str(outdim)+"_nitr"+str(n_iter)+"_batch"+str(M)+\
                "_thold"+str(thold)+"_pU"+str(args.priorU)+"_pV"+str(args.priorV)+\
                "_alpha"+str(args.alpha)+"_cas_V.csv", V_out)
            U_vadout = pf.transform(vad_data_tr_coo, vad_data_tr_coo.row, vad_data_tr_coo.col).copy()
            np.savetxt(OUT_DATA_DIR + '/'+ model_name + '_bin'+str(binary)+'_cauk0_'+str(caudim)\
                +'outK'+str(outdim)+"_nitr"+str(n_iter)+"_batch"+str(M)+\
                "_thold"+str(thold)+"_pU"+str(args.priorU)+"_pV"+str(args.priorV)+\
                "_alpha"+str(args.alpha)+"_cas_vadU.csv", U_vadout)
            U_testout = pf.transform(test_data_tr_coo, test_data_tr_coo.row, test_data_tr_coo.col).copy()
            np.savetxt(OUT_DATA_DIR + '/'+ model_name + '_bin'+str(binary)+'_cauk0_'+str(caudim)\
                +'outK'+str(outdim)+"_nitr"+str(n_iter)+"_batch"+str(M)+\
                "_thold"+str(thold)+"_pU"+str(args.priorU)+"_pV"+str(args.priorV)+\
                "_alpha"+str(args.alpha)+"_cas_testU.csv", U_testout)
            ...
            train_pos_plp[i], train_neg_plp[i], train_all_plp[i] = \
                log_cond_poisson_prob_metrics(train_data, pred_train)

            vad_pos_plp[i], vad_neg_plp[i], vad_all_plp[i] = \
                log_cond_poisson_prob_metrics(vad_data_te, pred_vad)

            test_pos_plp[i], test_neg_plp[i], test_all_plp[i] = \
                log_cond_poisson_prob_metrics(test_data_te, pred_test)
